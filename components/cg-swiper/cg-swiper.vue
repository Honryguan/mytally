			...
			<swiper-item style="width: 100%;height: 360upx;">
				<view class="swiper-item">
					<view class="swiper-item-view" v-for="(item, index) in swiperList" :key="index" v-if="index < 10" @click="clickItem(item)">
						...
						<uni-icons class="swiper-img" :type="item.icon" :color="activeIndex === item.id?'#4cd964':item.color" size="40" />
						<text :style="{color:activeIndex === item.id?'#4cd964':item.color}">{{ item.text }}</text>
					</view>
				</view>
			</swiper-item>
